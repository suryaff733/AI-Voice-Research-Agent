...
export async function POST(req: NextRequest) {
  try {
    ...
    const filename = `context_research_engine_${format}_${Date.now()}`;

    if (format === 'markdown') {
      const content = `# Context Research Engine Report\n\n**Question:** ${report.question}\n\n**Answer:** ${report.answer}\n\n## Summary\n${report.summary}\n\n## Sources\n${report.sources.map((s: any) => `- [${s.title}](${s.url}) (Credibility: ${s.credibility.score}/100)`).join('\n')}\n`;
    ...
    const textContent = `Context Research Engine document compiled in ${format.toUpperCase()} format.\nReport Topic: ${report.question}\nConfidence: ${report.confidence}%\n`;
